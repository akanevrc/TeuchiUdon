...
namespace akanevrc.TeuchiUdon.Tests.Editor
{
    ...
    public class CompileTeuchiUdonScriptTest
    {
        ...
        public void TeuchiUdonCodeCompiled()
        {
            ...
            if (expected == "")
            {
                program = TeuchiUdonUnityCompilerRunner.SaveTeuchiUdonAsset(srcAssetPath, binAssetPath);
            }
            else if (expected == "error")
            {
                Assert.That
                (
                    () => TeuchiUdonUnityCompilerRunner.SaveTeuchiUdonAsset(srcAssetPath, binAssetPath),
